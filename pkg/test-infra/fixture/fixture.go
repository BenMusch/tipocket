...
func init() {
	...
	flag.StringVar(&Context.TiDBClusterConfig.TiDBMonitorInitializerImage, "monitor-initializer-image", "pingcap/tidb-monitor-initializer:nightly", "tidb-monitor image. if no tag specified in the image, defaults to 'latest'")
	flag.StringVar(&Context.TiDBClusterConfig.TiDBMonitorReloaderImage, "monitor-reloader-image", "pingcap/tidb-monitor-reloader:v1.0.1", "tidb-monitor-reloader image. if no tag specified in the image, defaults to 'latest'")
	flag.StringVar(&Context.TiDBClusterConfig.GrafanaImage, "grafana-image", "grafana/grafana:6.0.1", "grafana image, defaults to latest if untagged")
	flag.StringVar(&Context.TiDBClusterConfig.PrometheusImage, "prometheus-image", "prom/prometheus:v2.11.1", "prometheus image, defaults to latest if untagged")
